...
struct ApprovalsView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: Theme.Space.md) {
                if poller.pending.isEmpty {
                    Panel(title: L("待处理", "Pending")) {
                        EmptyNote(text: "✅ " + L("没有待处理的审批", "No pending approvals"))
                    }
                } else {
                    Panel(title: L("待处理", "Pending"), subtitle: L("\(poller.pending.count) 条", "\(poller.pending.count)")) {
                ...
                if !resolved.isEmpty {
                    Panel(title: L("已处理", "Resolved"), subtitle: L("最近 \(min(resolved.count, 30)) 条", "last \(min(resolved.count, 30))")) {
                        VStack(spacing: Theme.Space.sm) {
        ...
        .pageBackground()
        .navigationTitle(L("审批", "Approvals"))
    }
    ...
    private func row(_ item: Approval, actionable: Bool) -> some View {
        HStack(alignment: .top, spacing: Theme.Space.md) {
            ...
            VStack(alignment: .leading, spacing: Theme.Space.xs) {
                HStack(spacing: Theme.Space.sm) {
                    ...
                    if !actionable, let status = item.status {
                        Pill(text: status == "approved" ? L("已通过", "Approved") : L("已拒绝", "Rejected"),
            ...
            if actionable {
                HStack(spacing: Theme.Space.sm) {
                    Button("✓ " + L("通过", "Approve")) { poller.resolveApproval(item.id, action: "approve") }
                        .buttonStyle(BrandButtonStyle(
                            tint: LinearGradient(colors: [Theme.green, Theme.green.opacity(0.75)],
                                                 startPoint: .top, endPoint: .bottom)))
                    Button("✗ " + L("拒绝", "Reject")) { poller.resolveApproval(item.id, action: "reject") }
