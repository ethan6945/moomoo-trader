...
def check_gemini() -> tuple[str, str]:
    """('ok'|'bad'|'skip', detail). 'bad' ONLY on owner-actionable failures
    (quota/balance, invalid key, geo-block). Transient 503 / no-key → 'skip'."""
    keys = list(settings.gemini_keys)
    if not keys:
        return "skip", "no Gemini key configured"
    from google import genai
    last_err = ""
    for key in keys:
        try:
            c = genai.Client(api_key=key)
            c.models.generate_content(model=settings.gemini_model, contents="ping")
            return "ok", "ok"
        except Exception as e:
            last_err = str(e)
            el = last_err.lower()
            if any(x in el for x in ("503", "500", "unavailable", "deadline")):
                return "skip", f"transient ({last_err[:60]})"
            continue                    # try next key on quota/auth, classify after
    el = last_err.lower()
    if "429" in last_err or "resource_exhausted" in el or "quota" in el:
        return "bad", f"配额/余额耗尽 ({last_err[:90]})"
    if any(x in el for x in ("api key", "permission_denied", "unauthenticated",
                             "401", "403", "invalid argument: api")):
...
def run(client=None) -> None:
    ...
    # --- Gemini API ---
    try:
        gem_status, gem_detail = check_gemini()
    except Exception as e:
        gem_status, gem_detail = "skip", f"check error ({e})"
    log.info("health: gemini=%s (%s)", gem_status, gem_detail)
    _edge(_K_GEM, gem_status,
          fail_msg=("⚠️ *Gemini API 不可用*：" + gem_detail +
                    "\nAI 层会 fail-safe（不会乱动仓位），但智能退出/情绪选股会变盲。"
                    "请尽快充值或检查 API key。"),
          recover_msg="✅ Gemini API 已恢复正常。")
